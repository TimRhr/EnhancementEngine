...
class SideEffect:
    ...
    evidence_level: str = (
        "theoretical"  # "theoretical", "animal_studies", "human_case", "clinical_trial"
    )
...
class EnhancementReport:
    ...
    def feasibility_score(self) -> float:
        ...
        effect_score = min(
            100, self.predicted_effect.enhancement_gain.improvement_factor * 50
        )
    ...
    def summary(self) -> str:
        ...
        return (
            f"Enhancement of {gene} via {variant}: "
...
class BatchReport:
    ...
    failed_analyses: Dict[str, str] = field(
    ...
    def top_candidates(self) -> List[Tuple[str, float]]:
        ...
        candidates = [
            (gene, report.feasibility_score) for gene, report in self.results.items()
        ]
        return sorted(candidates, key=lambda x: x[1], reverse=True)

    def get_by_category(
        self, category: EnhancementCategory
    ) -> Dict[str, EnhancementReport]:
