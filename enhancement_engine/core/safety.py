...
from ..models.constants import DEFAULT_PARAMETERS, SCORING_THRESHOLDS, ERROR_MESSAGES
...
class EssentialGeneDatabase:
    ...
    def __init__(self):
        ...
        self.essential_categories = {
            ...
            "cell_cycle": {
                "weight": 2.5,
                "genes": {"CDKN1A", "RB1", "E2F1", "CHEK1", "CHEK2"},
            },
            "transcription": {
                "weight": 2.0,
                "genes": {"POLR2A", "POLR2B", "TAF1", "TBP"},
            },
    ...
    def analyze_safety(
        ...
        try:
            ...
            off_target_analysis = self.off_target_analyzer.analyze_off_targets(
                guide_rna
            )
            ...
            immunogenicity_analysis = (
                self.immunogenicity_predictor.predict_immunogenicity(
            ...
            genotoxicity_score = max(
                0, 100 - (genotoxicity_analysis["overall_score"] * 10)
            )
            immunogenicity_score = max(
                0, 100 - (immunogenicity_analysis["overall_score"] * 10)
            )
            ...
            self.logger.info(
                f"Safety analysis completed for {target_gene}: {overall_score:.1f}/100"
            )
            return safety_score
    ...
    def _calculate_essential_gene_score(
        self, essential_analysis: Dict[str, Any]
    ) -> float:
    ...
    def _calculate_confidence_level(
        ...
        delivery_score = immunogenicity_analysis.get("delivery_immunogenicity", {}).get(
            "score", 0
        )
        if delivery_score > 5:
            base_confidence -= 0.1
    ...
    def generate_comprehensive_safety_report(
        self,
        guide_rna: GuideRNA,
        target_gene: str,
        modification_type: str = "knockout",
        ...
        safety_score = self.analyze_safety(
            guide_rna, target_gene, modification_type, delivery_method
        )
    ...
    def _generate_overall_recommendations(self, safety_score: SafetyScore) -> List[str]:
        ...
        if safety_score.overall_score >= 90:
            recommendations.append(
                "Excellent safety profile - proceed with standard protocols"
            )
        elif safety_score.overall_score >= 70:
            recommendations.append(
                "Good safety profile - proceed with enhanced monitoring"
            )
        elif safety_score.overall_score >= 50:
            recommendations.append(
                "Moderate safety concerns - consider risk mitigation strategies"
            )
        elif safety_score.overall_score >= 30:
            recommendations.append(
                "Significant safety concerns - extensive evaluation required"
            )
        else:
        ...
        if safety_score.off_target_score < 50:
            recommendations.append(
                "Off-target risk is primary concern - validate experimentally"
            )

        if safety_score.genotoxicity_score < 50:
            recommendations.append(
                "Genotoxicity risk elevated - monitor chromosomal stability"
            )

        if safety_score.immunogenicity_score < 50:
            recommendations.append(
                "Immunogenicity risk high - consider immunosuppression"
            )

        if safety_score.essential_gene_score < 50:
            recommendations.append(
                "Essential gene impact detected - critical safety evaluation needed"
            )
        ...
        if off_target_analysis["essential_analysis"]["total_essential_hits"] > 0:
            side_effects.append(
                SideEffect(
                    ...
                    probability=0.3
                    * off_target_analysis["essential_analysis"]["total_essential_hits"],
                    severity="severe",
                    reversible=False,
                    onset_time="days",
                    evidence_level="theoretical",
        ...
        if immunogenicity_analysis["overall_score"] > 4.0:
            side_effects.append(
                SideEffect(
                    ...
                    probability=min(
                        0.7, immunogenicity_analysis["overall_score"] / 8.0
                    ),
                    severity="moderate",
                    reversible=True,
                    onset_time="days",
    ...
    def _generate_monitoring_requirements(self, safety_score: SafetyScore) -> List[str]:
        ...
        if safety_score.off_target_score < 70:
            monitoring.append(
                "Periodic whole-genome sequencing to detect off-target effects"
            )
        ...
        if safety_score.overall_score < 50:
            monitoring.append(
                "Weekly monitoring for first month, then monthly for 6 months"
            )
        elif safety_score.overall_score < 70:
            monitoring.append(
                "Bi-weekly monitoring for first month, then monthly for 3 months"
            )
        else:
    ...
    def _generate_risk_mitigation_strategies(
        ...
        if off_target_analysis["risk_score"] > 2.0:
            ...
            strategies.append(
                "Implement dual-guide approaches for increased specificity"
            )
        ...
        if immunogenicity_analysis["overall_score"] > 4.0:
            ...
            strategies.append(
                "Use alternative delivery methods with lower immunogenicity"
            )
    ...
    def compare_guide_safety(
        self, guides: List[GuideRNA], target_gene: str
    ) -> List[Tuple[GuideRNA, SafetyScore]]:
        """
        ...
        for guide in guides:
            try:
                ...
                self.logger.warning(
                    f"Failed to analyze safety for guide {guide.sequence}: {e}"
                )
    ...
    def assess_enhancement_safety(
        self,
        target_gene: str,
        enhancement_category: EnhancementCategory,
        modification_type: str = "base_edit",
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "monitoring_duration": self._get_enhancement_monitoring_duration(
    ...
    def _get_enhancement_monitoring_duration(
    ...
    def _assess_enhancement_reversibility(
        self, target_gene: str, modification_type: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "reversibility_level": (
                "High"
                if overall_reversibility > 0.7
                else "Moderate" if overall_reversibility > 0.4 else "Low"
            ),
    ...
    def _get_reversal_strategies(self, modification_type: str) -> List[str]:
        ...
        strategies = {
            ...
            "activation": ["Remove activating elements", "Counter-repressions"],
            "interference": ["Discontinue interfering RNA", "Natural RNA degradation"],
        }

        return strategies.get(
            modification_type, ["Consult genetic counselor for reversal options"]
        )
...
class OffTargetAnalyzer:
    ...
    def analyze_off_targets(self, guide_rna: GuideRNA) -> Dict[str, Any]:
        ...
        risk_score = self._calculate_off_target_risk_score(
            off_targets, essential_analysis
        )
        ...
        return {
            ...
            "chromosome_distribution": self._analyze_chromosome_distribution(
                off_targets
            ),
        }
    ...
    def _analyze_essential_gene_impacts(
        self, off_targets: List[OffTarget]
    ) -> Dict[str, Any]:
        ...
        for hit in essential_hits:
            if hit.gene_context:
                category = self.essential_db.get_gene_function_category(
                    hit.gene_context
                )
                if category:
        ...
        critical_hits = [
            ...
            for hit in essential_hits
            if hit.mismatches <= 1
            and self.essential_db.get_essentiality_score(hit.gene_context or "") >= 2.0
        ]

        # Calculate highest risk score
        risk_scores = [
            (1.0 / (1.0 + hit.mismatches))
            * self.essential_db.get_essentiality_score(hit.gene_context or "")
            for hit in essential_hits
            if hit.gene_context
        ]
    ...
    def _calculate_off_target_risk_score(
        self, off_targets: List[OffTarget], essential_analysis: Dict[str, Any]
    ) -> float:
        ...
        for ot in off_targets:
            ...
            if ot.essential_gene and ot.gene_context:
                essentiality_score = self.essential_db.get_essentiality_score(
                    ot.gene_context
                )
    ...
    def _generate_off_target_recommendations(
        self,
        ...
        if risk_score < 0.5:
            ...
            recommendations.append(
                "Moderate off-target risk - enhanced monitoring required"
            )
            recommendations.append(
                "Consider experimental validation of predicted off-targets"
            )
        else:
            recommendations.append("High off-target risk - consider alternative guides")
            recommendations.append(
                "Experimental off-target validation strongly recommended"
            )
        ...
        if risk_categories["high"] > 3:
            recommendations.append(
                "Consider guides with fewer single-mismatch off-targets"
            )

        return recommendations

    def _analyze_chromosome_distribution(
        self, off_targets: List[OffTarget]
    ) -> Dict[str, int]:
...
class GenotoxicityAssessor:
    ...
    def assess_genotoxicity(
        self, guide_rna: GuideRNA, target_gene: str, modification_type: str = "knockout"
    ) -> Dict[str, Any]:
        """
    ...
    def _assess_chromosomal_instability(self, guide_rna: GuideRNA) -> Dict[str, Any]:
        ...
        return {
            "score": min(5.0, total_risk),
            "multi_hit_chromosomes": {
                k: v for k, v in chromosome_hits.items() if v > 1
            },
            "repetitive_hits": repetitive_risk / 0.2 if repetitive_risk > 0 else 0,
        }

    def _assess_mutagenesis_risk(
        self, modification_type: str, target_gene: str
    ) -> Dict[str, Any]:
    ...
    def _calculate_genotoxicity_score(
        self, dna_damage: Dict, chromosomal: Dict, mutagenesis: Dict
    ) -> float:
        """Calculate overall genotoxicity score."""
        # Weighted combination of risk factors
        weights = {"dna_damage": 0.4, "chromosomal": 0.3, "mutagenesis": 0.3}
    ...
    def _generate_genotoxicity_recommendations(
        self,
        overall_score: float,
        dna_damage: Dict,
        chromosomal: Dict,
        ...
        if overall_score < 1.0:
            recommendations.append(
                "Low genotoxicity risk - standard monitoring sufficient"
            )
        elif overall_score < 2.5:
            recommendations.append(
                "Moderate genotoxicity risk - regular safety monitoring"
            )
        elif overall_score < 5.0:
            recommendations.append(
                "Elevated genotoxicity risk - enhanced monitoring required"
            )
            recommendations.append("Consider periodic genomic stability assessments")
        else:
            recommendations.append(
                "High genotoxicity risk - extensive safety evaluation needed"
            )
        ...
        if mutagenesis["high_risk_gene"]:
            recommendations.append(
                "Targeting tumor suppressor gene - cancer risk assessment needed"
            )
...
class ImmunogenicityPredictor:
    ...
    def predict_immunogenicity(
        self, guide_rna: GuideRNA, cas_type: CasType, delivery_method: str = "LNP"
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_immunogenicity_score(
        self, cas_immun: Dict, guide_immun: Dict, delivery_immun: Dict
    ) -> float:
        """Calculate overall immunogenicity score."""
    ...
    def _generate_immunogenicity_recommendations(
        self, overall_score: float, cas_immun: Dict, delivery_immun: Dict
    ) -> List[str]:
        ...
        if overall_score < 2.0:
            ...
            recommendations.append(
                "Moderate immunogenicity - monitor for immune responses"
            )
        elif overall_score < 6.0:
            recommendations.append(
                "Elevated immunogenicity - immunosuppression may be needed"
            )
        else:
            recommendations.append(
                "High immunogenicity risk - consider alternative approaches"
            )

        # Specific recommendations
        if cas_immun["score"] > 5.0:
            recommendations.append(
                "Consider using smaller Cas variants or base editors"
            )
        ...
        if delivery_immun["score"] > 5.0:
            ...
            if "virus" in str(delivery_immun.get("additional_factors", {})):
                recommendations.append(
                    "Viral delivery detected - monitor for anti-viral responses"
                )
