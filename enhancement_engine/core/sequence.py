...
class VariantAnalyzer:
    ...
    def find_variant_position(
        self, sequence: SeqRecord, variant: VariantInfo
    ) -> Optional[VariantPosition]:
        """
        ...
        try:
            ...
            if seq_position >= len(sequence.seq):
                self.logger.warning(
                    f"Variant position {seq_position} outside sequence length {len(sequence.seq)}"
                )
                return None
...
class SequenceAnalyzer:
        ...
        for start_pos in start_positions:
            for i in range(start_pos, len(seq_str) - 2, 3):
                ...
                if codon in ["TAA", "TAG", "TGA"]:
                    ...
                    return CodingRegion(
                        start=start_pos, end=end_pos, exons=exons, introns=[]
                    )
    ...
    def analyze_sequence_composition(self, sequence: Union[str, Seq]) -> Dict[str, Any]:
        ...
        composition_pct = {
            base: count / total_bases * 100 for base, count in composition.items()
        }
    ...
    def _find_repeats(
        self, sequence: str, min_length: int = 4, max_length: int = 20
    ) -> List[Dict[str, Any]]:
    ...
    def _find_hairpins(
        self, sequence: str, min_stem: int = 6, max_loop: int = 10
    ) -> List[Dict[str, Any]]:
        """Find potential hairpin structures."""
        hairpins = []
    ...
    def _find_inverted_repeats(
        self, sequence: str, min_length: int = 8
    ) -> List[Dict[str, Any]]:
    ...
    def compare_sequences(
        self, seq1: Union[str, Seq], seq2: Union[str, Seq]
    ) -> Dict[str, Any]:
        """
