...
from ..models.therapeutic_data_classes import (
    TherapeuticReport, TherapeuticTarget, CorrectionStrategy,
    TherapeuticEfficacy, TherapeuticSafety, PatientStratification,
    DiseaseRisk, TherapeuticApproach, DeliveryMethod, DiseaseCategory,
    ClinicalEndpoint, ReversalStrategy
)
from ..models.disease_constants import (
    DISEASE_GENES, THERAPEUTIC_SAFETY_THRESHOLDS, DISEASE_SCORING_SYSTEMS
)
...
class TherapeuticEnhancementEngine:
    ...
    def analyze_disease_gene(self, gene_name: str, variant: str,
                           patient_data: Dict[str, Any],
                           disease: str = "rheumatoid_arthritis") -> TherapeuticReport:
        """
        ...
        """
        try:
            self.logger.info(f"Starting therapeutic analysis of {gene_name} {variant} for {disease}")
            
            # Check cache first
            cache_key = f"therapeutic_{gene_name}_{variant}_{disease}_{hash(str(patient_data))}"
            ...
            disease_risk = self._assess_disease_risk(gene_name, variant, patient_data, disease)
            ...
            recommendations = self._generate_therapeutic_recommendations(
                disease_risk, safety_assessment, predicted_efficacy, patient_stratification
            )
            ...
            therapeutic_report = TherapeuticReport(
                ...
                follow_up_schedule=self._generate_follow_up_schedule(predicted_efficacy),
    ...
    def _assess_disease_risk(self, gene_name: str, variant: str,
                           patient_data: Dict[str, Any], disease: str) -> DiseaseRisk:
        ...
        try:
            ...
            self.logger.debug(f"Disease risk calculated: OR={disease_risk.odds_ratio:.2f}")
            return disease_risk
    ...
    def _stratify_patient(self, patient_data: Dict[str, Any], 
                         disease_risk: DiseaseRisk) -> PatientStratification:
        """Stratify patient for treatment candidacy."""
        genotype_data = {gene: variant for gene, variant in patient_data.get("genotypes", {}).items()}
        
        # Determine disease severity
        disease_severity = patient_data.get("disease_severity", "moderate")
        
        # Calculate treatment priority based on risk and severity
        treatment_priority = self._calculate_treatment_priority(disease_risk, disease_severity)
    ...
    def _calculate_treatment_priority(self, disease_risk: DiseaseRisk, 
                                    disease_severity: str) -> float:
    ...
    def _identify_therapeutic_targets(self, gene_name: str, variant: str,
                                    disease: str, patient_strat: PatientStratification) -> List[TherapeuticTarget]:
        ...
        therapeutic_approach = self._determine_therapeutic_approach(gene_name, variant, gene_data)
        
        # Determine target tissue
        target_tissue = self._determine_target_tissue(disease, gene_name)
        
        # Determine delivery method
        delivery_method = self._determine_delivery_method(target_tissue, therapeutic_approach)
        
        # Get correction sequence if needed
        correction_sequence = self._get_correction_sequence(gene_name, variant, therapeutic_approach)
    ...
    def _determine_therapeutic_approach(self, gene_name: str, variant: str,
                                      gene_data: Dict[str, Any]) -> TherapeuticApproach:
    ...
    def _determine_delivery_method(self, target_tissue: str, 
                                 therapeutic_approach: TherapeuticApproach) -> DeliveryMethod:
    ...
    def _get_correction_sequence(self, gene_name: str, variant: str,
                               approach: TherapeuticApproach) -> Optional[str]:
    ...
    def _design_correction_strategies(self, targets: List[TherapeuticTarget],
                                    gene_name: str) -> List[CorrectionStrategy]:
        ...
        for target in targets:
            try:
                ...
                self.logger.warning(f"Failed to design strategy for {target.gene_symbol}: {e}")
    ...
    def _get_target_sequence(self, gene_name: str) -> Optional[str]:
        ...
        try:
            ...
            if gene_info and gene_info.refseq_id:
                sequence_record = self.base_engine.gene_database.get_sequence(gene_info.refseq_id)
                if sequence_record:
    ...
    def _assess_therapeutic_safety(self, targets: List[TherapeuticTarget],
    ...
    def _predict_therapeutic_efficacy(self, strategies: List[CorrectionStrategy],
                                    targets: List[TherapeuticTarget],
                                    patient_strat: PatientStratification) -> TherapeuticEfficacy:
    ...
    def _define_clinical_endpoints(self, disease: str, patient_strat: PatientStratification,
                                 efficacy: TherapeuticEfficacy) -> List[ClinicalEndpoint]:
        ...
        if disease == "rheumatoid_arthritis":
            ...
            endpoints.append(ClinicalEndpoint(
                endpoint_name="DAS28",
    ...
    def _generate_therapeutic_recommendations(self, disease_risk: DiseaseRisk,
                                            safety: TherapeuticSafety,
                                            efficacy: TherapeuticEfficacy,
                                            patient_strat: PatientStratification) -> List[str]:
        ...
        if overall_benefit > 0.7 and overall_risk < 0.3:
            ...
            recommendations.append("Consider treatment if conventional therapies have failed")
        else:
        ...
        if efficacy.correction_efficiency < 0.3:
            recommendations.append("Consider multiple treatment cycles for optimal effect")
        ...
        if patient_strat.treatment_priority > 0.8:
            recommendations.append("High priority case - expedited treatment consideration")
    ...
    def _identify_contraindications(self, safety: TherapeuticSafety,
                                  patient_data: Dict[str, Any],
                                  disease_risk: DiseaseRisk) -> List[str]:
    ...
    def _generate_monitoring_plan(self, safety: TherapeuticSafety,
                                efficacy: TherapeuticEfficacy,
                                targets: List[TherapeuticTarget]) -> List[str]:
    ...
    def _generate_follow_up_schedule(self, efficacy: TherapeuticEfficacy) -> Dict[str, str]:
        ...
        if efficacy.persistence_months < 6:
            schedule["2_months"] = "Early efficacy assessment for potential re-treatment"
        
        return schedule
    
    def _calculate_therapeutic_confidence(self, safety: TherapeuticSafety,
                                        efficacy: TherapeuticEfficacy,
                                        num_strategies: int) -> float:
        ...
        strategy_confidence = min(1.0, num_strategies / 3)  # More strategies = higher confidence
    ...
    def calculate_polygenic_risk(self, genotype_data: Dict[str, str],
                               patient_data: Dict[str, Any],
                               disease: str = "rheumatoid_arthritis") -> Dict[str, Any]:
        """
        ...
        """
        try:
            genetic_risk_score = self.multigene_risk_calculator.calculate_polygenic_risk_score(
                genotype_data, patient_data, disease
            )
            
            # Generate therapeutic recommendations for polygenic risk
            therapeutic_recommendations = self._generate_polygenic_therapeutic_recommendations(
    ...
    def _generate_polygenic_therapeutic_recommendations(self, genetic_risk_score, 
    ...
    def _identify_priority_therapeutic_targets(self, individual_contributions: Dict[str, float]) -> List[str]:
        """Identify priority genes for therapeutic targeting."""
        # Sort genes by contribution to risk
        sorted_genes = sorted(individual_contributions.items(), key=lambda x: x[1], reverse=True)
        
        # Return top 3 contributors
        return [gene for gene, contribution in sorted_genes[:3] if contribution > 1.5]
    
    def _design_polygenic_intervention_strategy(self, genotype_data: Dict[str, str],
                                              genetic_risk_score) -> Dict[str, Any]:
        ...
        for gene in priority_targets:
            ...
            if contribution > 1.5:
                total_reduction += (contribution - 1.0) * 0.7  # 70% correction efficiency
    ...
    def compare_therapeutic_options(self, gene_list: List[str],
                                  patient_data: Dict[str, Any],
                                  disease: str = "rheumatoid_arthritis") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for gene in gene_list:
                ...
                if variant:
                    try:
                        analysis = self.analyze_disease_gene(gene, variant, patient_data, disease)
    ...
    def _perform_therapeutic_comparison(self, analyses: Dict[str, TherapeuticReport]) -> Dict[str, Any]:
        ...
        # Rank options
        rankings = {
            "by_safety": sorted(analyses.keys(), 
                              key=lambda x: comparison_data[x]["safety_score"], reverse=True),
            "by_efficacy": sorted(analyses.keys(),
                                key=lambda x: comparison_data[x]["efficacy_score"], reverse=True),
            "by_overall_score": sorted(analyses.keys(),
                                     key=lambda x: (comparison_data[x]["safety_score"] * 0.6 + 
                                                   comparison_data[x]["efficacy_score"] * 0.4), reverse=True)
        }
        
        # Generate recommendations
        best_option = rankings["by_overall_score"][0] if rankings["by_overall_score"] else None
        
        return {
            "comparison_data": comparison_data,
            "rankings": rankings,
            "best_overall_option": best_option,
            "recommendation": self._generate_comparison_recommendation(comparison_data, rankings),
            "summary_statistics": self._calculate_comparison_statistics(comparison_data)
        }
    
    def _generate_comparison_recommendation(self, comparison_data: Dict, rankings: Dict) -> str:
    ...
    def _calculate_comparison_statistics(self, comparison_data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommended_options": sum(1 for gene, data in comparison_data.items() 
                                     if "recommend" in data["treatment_recommendation"].lower())
        }
    
    def export_therapeutic_report(self, report: TherapeuticReport, 
                                format: str = "json") -> str:
        """
