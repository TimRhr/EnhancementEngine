...
class TherapeuticOffTargetAnalyzer:
    ...
    def analyze_therapeutic_off_targets(
        self,
        guide_rna: GuideRNA,
        target_disease: str,
        ...
        try:
            ...
            therapeutic_analysis.update(
                {
                    ...
                    "clinical_monitoring": self._generate_monitoring_plan(
                        therapeutic_risks
                    ),
                    "patient_counseling": self._generate_counseling_points(
                        therapeutic_risks
                    ),
                }
            )
            ...
            raise TherapeuticSafetyError(
                f"Failed to analyze therapeutic off-targets: {e}"
            )

    def _assess_therapeutic_off_target_risks(
        self,
        off_targets: List,
        target_disease: str,
        correction_strategy: CorrectionStrategy,
        ...
        if correction_strategy.editing_method == "base_editing":
            risks["bystander_editing"] = self._assess_bystander_editing_risk(
                off_targets
            )
        elif correction_strategy.editing_method == "HDR":
            risks["integration_errors"] = self._assess_integration_error_risk(
                off_targets
            )

        return risks

    def _assess_autoimmune_off_target_risks(
        ...
        for ot in off_targets:
            if ot.gene_context and any(
                gene in ot.gene_context.upper() for gene in autoimmune_genes
            ):
    ...
    def _assess_cardiovascular_off_target_risks(
        self, off_targets: List
    ) -> Dict[str, float]:
        """Assess off-target risks for cardiovascular interventions."""
        cardio_genes = {"APOB", "LDLR", "PCSK9", "HMGCR", "NPC1L1", "ABCG8", "CYP7A1"}
        ...
        for ot in off_targets:
            if ot.gene_context and any(
                gene in ot.gene_context.upper() for gene in cardio_genes
            ):
    ...
    def _evaluate_intervention_consequences(
        self, off_targets: List, editing_method: str
    ) -> Dict[str, Any]:
    ...
    def _generate_therapeutic_recommendations(
        self, base_analysis: Dict, therapeutic_risks: Dict, consequences: Dict
    ) -> List[str]:
        ...
        if therapeutic_risks.get("bystander_editing", 0) > 0.1:
            recommendations.append(
                "Use high-fidelity base editors to minimize bystander edits"
            )
    ...
    def _generate_monitoring_plan(self, therapeutic_risks: Dict) -> Dict[str, Any]:
        ...
        if therapeutic_risks.get("immune_complications", 0) > 0.2:
            ...
            monitoring_plan["short_term_monitoring"].append(
                "Weekly immune function tests"
            )
...
class TherapeuticGenotoxicityAssessor:
    ...
    def assess_therapeutic_genotoxicity(
        self,
        correction_strategy: CorrectionStrategy,
        target_tissue: str,
        ...
        try:
            ...
            raise TherapeuticSafetyError(
                f"Failed to assess therapeutic genotoxicity: {e}"
            )

    def _assess_tissue_specific_genotoxicity(
        self, target_tissue: str, editing_method: str
    ) -> Dict[str, float]:
    ...
    def _assess_delivery_genotoxicity(
        self, delivery_method: DeliveryMethod, editing_method: str
    ) -> Dict[str, float]:
    ...
    def _assess_patient_specific_genotoxicity(
        self, patient_factors: Dict[str, Any], correction_strategy: CorrectionStrategy
    ) -> Dict[str, float]:
    ...
    def _assess_integration_genotoxicity(
        ...
        if correction_strategy.editing_method == "HDR":
            ...
            if (
                correction_strategy.template_sequence
                and len(correction_strategy.template_sequence) > 1000
            ):
    ...
    def _calculate_cumulative_genotoxicity(
        self,
...
class TherapeuticSafetyAnalyzer:
    ...
    def analyze_therapeutic_safety(
        self,
        ...
        try:
            ...
            primary_guide = (
                best_strategy.guide_rnas[0] if best_strategy.guide_rnas else None
            )
            ...
            if primary_guide:
                off_target_analysis = (
                    self.therapeutic_off_target.analyze_therapeutic_off_targets(
                        primary_guide,
                        therapeutic_target.disease_association,
                        best_strategy,
                    )
                )
            ...
            genotoxicity_analysis = (
                self.therapeutic_genotoxicity.assess_therapeutic_genotoxicity(
                    best_strategy,
                    therapeutic_target.target_tissue,
                    therapeutic_target.delivery_method,
                    patient_data,
                )
            )
            ...
            reversibility_score = self._assess_reversibility(
                best_strategy, therapeutic_target
            )
            ...
            recommendations = self._generate_therapeutic_safety_recommendations(
                ...
                genotoxicity_risk,
                immunogenicity_risk,
                tissue_toxicity_risk,
                systemic_effects_risk,
            )
            ...
            self.logger.info(
                f"Therapeutic safety analysis completed: {therapeutic_safety.risk_category}"
            )
    ...
    def _assess_therapeutic_immunogenicity(
        self,
        correction_strategy: CorrectionStrategy,
        therapeutic_target: TherapeuticTarget,
        patient_data: Dict[str, Any],
        ...
        delivery_modifier = delivery_modifiers.get(
            therapeutic_target.delivery_method, 1.0
        )
    ...
    def _assess_systemic_effects(
        ...
        elif therapeutic_target.delivery_method in [
            DeliveryMethod.INTRA_ARTICULAR,
            DeliveryMethod.LOCAL_INJECTION,
        ]:
    ...
    def _identify_contraindications(
        ...
        if "severe_immunodeficiency" in comorbidities:
            contraindications.append(
                "Severe immunodeficiency - unpredictable immune response"
            )
        ...
        if age < 16:
            contraindications.append(
                "Pediatric population - developmental considerations"
            )
        ...
        if therapeutic_target.disease_association == "rheumatoid_arthritis":
            if "severe_joint_destruction" in comorbidities:
                contraindications.append(
                    "Advanced joint destruction - limited benefit expected"
                )

        return contraindications

    def design_reversal_strategy(
        self,
        original_strategy: CorrectionStrategy,
        therapeutic_target: TherapeuticTarget,
    ) -> Optional[ReversalStrategy]:
        ...
        try:
            ...
            reversal_guides = self._design_reversal_guides(
            ...
            reversal_risks = self._identify_reversal_risks(
                original_strategy, reversal_method
            )
    ...
    def _determine_reversal_method(
        self, original_strategy: CorrectionStrategy
    ) -> Optional[str]:
    ...
    def _design_reversal_guides(
        self, original_strategy: CorrectionStrategy, reversal_method: str
    ) -> List[GuideRNA]:
    ...
    def _estimate_reversal_success(
        self, original_strategy: CorrectionStrategy, reversal_method: str
    ) -> float:
