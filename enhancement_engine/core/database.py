...
from ..models.data_classes import (
    GeneInfo, VariantInfo, EnhancementCategory, CasType
)
from ..models.constants import (
    ENHANCEMENT_GENES, NCBI_DATABASES, EXTERNAL_RESOURCES, ERROR_MESSAGES
)
...
class NCBIClient:
    ...
    def get_sequence(self, accession: str) -> SeqRecord:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved sequence for {accession}: {len(seq_record)} bp")
            return seq_record
...
class GeneDatabase:
    ...
    def __init__(self, email: str, cache_enabled: bool = True, 
    ...
    def search_gene(self, gene_name: str, organism: str = "homo sapiens") -> Optional[GeneInfo]:
        """
    ...
    def _parse_ncbi_gene(self, gene_data: Dict) -> GeneInfo:
        ...
        try:
            ...
            return GeneInfo(
                name=description,
                symbol=symbol,
                gene_id=str(entrezgene.get("Entrezgene_track-info", {}).get("Gene-track_geneid", "")),
    ...
    def get_enhancement_genes(self, category: Optional[EnhancementCategory] = None) -> Dict[str, GeneInfo]:
        """
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        for gene_info in self.enhancement_genes.values():
            ...
            stats["enhancement_categories"][category] = stats["enhancement_categories"].get(category, 0) + 1
        ...
        if self.cache_enabled:
            try:
                ...
                stats["cache_size_mb"] = sum(f.stat().st_size for f in cache_files) / (1024 * 1024)
            except Exception:
                stats["cache_files"] = 0
...
def get_enhancement_genes(category: Optional[str] = None, email: str = "temp@example.com") -> Dict[str, GeneInfo]:
...
def search_sequences(accessions: List[str], email: str) -> Dict[str, Optional[SeqRecord]]:
